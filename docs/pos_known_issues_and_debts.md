...
### ISSUE-POS-DUPLICATE-DIR-001：递归幽灵目录 `store_html/html/pos/pos/`（已处理）

- 类型：技术债 / 误操作残留
- 模块：POS · 文件结构

- 描述：
  - `store_html/html/pos/pos/` 是 `store_html/html/pos/` 的完整递归副本；
  - 经审计，该目录下的所有文件（API、资源、页面）均未被业务代码引用；
  - 这是一个典型的“复制粘贴误操作”产物，容易导致开发人员修改错误的文件。

- 修复措施（2025-11-21）：
  - 步骤 1：将 `store_html/html/pos/pos/` 重命名为 `store_html/html/pos/_pos_duplicated_20251121/`；
  - 步骤 2：在其中添加 `README_DO_NOT_USE.txt` 说明文件；
  - 后续计划：在确认无异常后，可在下一次清理任务中物理删除该目录。
